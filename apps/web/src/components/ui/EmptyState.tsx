...
export interface EmptyStateProps {
  /** Lucide icon element, e.g. `<Wallet />`. */
  icon?: ReactNode;
  title: string;
...
export function EmptyState({ icon, title, description, action, secondaryAction, className }: EmptyStateProps) {
  return (
    <div
      className={cn(
        "flex flex-col items-center justify-center gap-3 rounded-card border border-dashed border-line-strong bg-transparent px-6 py-12 text-center",
        className,
      )}
    >
      {icon ? (
